...
namespace AlarmWorkflow.BackendService.WebService.Modules
{
    public class Home : NancyModule
    {
        ...
        public Home(IServiceProvider serviceProvider)
        {
            ...
            Get["/display"] = _ =>
            {
                IList<EmkResource> emkResources = emkService.GetAllResources()
                    .Where(resource => resource.IsActive).ToList();

                PageSettings pageSettings = new PageSettings()
                {
                    EmkResources = JsonConvert.SerializeObject(emkResources, Formatting.Indented)
                };

                return View["display.html", pageSettings];
            };
        }
